...
% hs_img_bin = hs_img>0;
% 
% skel_img = slow_img_skel;
% skel_img_bin = skel_img>0;
% 
% ves_segs_img = labeled_vess_segs;
...
% 
% cd(main_dir_file_name)
% gfp_img_name = strcat(shortfile,'_iso_ch2.tif');
% gfp_img = loadtiff(gfp_img_name);
% tissue_thresh = mat2gray(gfp_img)>0.025;
% tissue_vol = sum(tissue_thresh(:));
% 
% even_vol_10vox = (dist_tform_hs_even<10).*tissue_thresh;
% actual_vol_10vox = (dist_tform_hs<10).*tissue_thresh;
% 
% even_hs_vol = sum(even_vol_10vox(:));
% actual_hs_vol = sum(actual_vol_10vox(:));
% 
% perct_vol_even = even_hs_vol/tissue_vol*100;
% perct_vol_actual = actual_hs_vol/tissue_vol*100;
% 
% result_test = [perct_vol_actual perct_vol_even];
% result_test_table_diff = array2table(result_test);
% result_test_table_diff.Properties.VariableNames = {'perct_vol_actual' 'perct_vol_even'};
% 
